...
Public Class FrmMain
    ...
    Private Sub btnFix_x64_Click(sender As Object, e As EventArgs) Handles btnFix_x64.Click
        ...
        Else
            ...
            If MessageBox.Show("您确定要进行修复程序吗？", "Ballamap", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes _
                And MessageBox.Show("请再次确认您的系统为 32 位。确认后将开始修复注册表项，请在接下来弹出的授权框中选择是。", "Ballamap", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub btnFix_x86_Click(sender As Object, e As EventArgs)
    ...
    Private Sub TxtPath_Click(sender As Object, e As EventArgs) Handles txtPath.Click
        MessageBox.Show("您的 Ballance 目录为：" & vbCrLf & txtPath.Text & vbCrLf & "需要更改请返回后选择浏览。", "Ballamap", MessageBoxButtons.OK, MessageBoxIcon.Information)
    End Sub
    ...
    Private Sub TabAbout_Leave(sender As Object, e As EventArgs) Handles TabAbout.Leave
        btnUpdateSetting.Visible = True
    End Sub

    Private Sub lstMap_SelectedIndexChanged(sender As Object, e As EventArgs) Handles lstMap.SelectedIndexChanged
        btnInstallNew.Enabled = True
        picDeleteNew.Visible = True
    End Sub
    '=============================== New Map Manager ======================================
    Dim ChosenLevel As Integer = 0

    Private Sub cmbLevel_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbLevel.SelectedIndexChanged
        ChosenLevel = Val(Mid(cmbLevel.SelectedItem, 7))
        MsgBox(ChosenLevel)
        Dim FileNames() As String = Directory.GetFiles(Application.StartupPath & "\Maps")
        Dim filename As String
        lstMap.Items.Clear()
        For i As Integer = 0 To FileNames.Length - 1
            filename = Path.GetFileName(FileNames(i))
            lstMap.Items.Add(filename)

        Next
    End Sub

    Private Sub picDeleteNew_Click(sender As Object, e As EventArgs) Handles picDeleteNew.Click
        If MsgBox("确认要删除" & lstMap.SelectedItem & "吗？", MsgBoxStyle.YesNo, "Ballamap") = MsgBoxResult.Yes Then
            My.Computer.FileSystem.DeleteFile(Application.StartupPath & "\Maps\" & lstMap.SelectedItem)
            lstMap.Items.Clear()
            picDeleteNew.Visible = False
            Dim FileNames() As String = Directory.GetFiles(Application.StartupPath & "\Maps")
            For i As Integer = 0 To FileNames.Length - 1
                lstMap.Items.Add(Path.GetFileName(FileNames(i)))
            Next
        End If
    End Sub

    Private Sub btnBrowseByExp_Click(sender As Object, e As EventArgs) Handles btnBrowseByExp.Click
        System.Diagnostics.Process.Start("explorer.exe", "/select " & Application.StartupPath & "\Maps")
    End Sub

    Private Sub btnInstallNew_Click(sender As Object, e As EventArgs) Handles btnInstallNew.Click
